...
def determinar_corredores(pedidos_bin, pedidos, corredores, start_time=None, max_tempo_segundos=None):
    ...
    for pid, included in enumerate(pedidos_bin): # usamos o array binário para percorrer e verificar se está incluído
        if included:
            for i, q in pedidos[pid].itens.items(): # usamos a lista de pedidos para buscar os itens do pedido do array binário
    ...
    for cid in corredores_ids:
        ...
        # condição de parada otimizada (evitar todas() toda hora)
        # só checar itens demandados
...
def gerar_solucao_inicial(pedidos, corredores, lb, ub, start_time=None, max_tempo_segundos=None, tentativas_max=1000):
    for tentativa in range(tentativas_max):
        ...
        if total_unidades < lb: # caso não esteja entre LB e UB, tenta mais uma vez
            continue
...
def perturbar(pedidos_bin, pedidos, lb, ub, intensidade=0.2):
    pedidos_novo = pedidos_bin[:] # cópia rasa
        ...
        for p_out in indices_in:
            ...
            for p_in in indices_out:
                ...
                if obj > melhor_obj:
                    ...
                    melhor_obj = obj
                    melhorou = True
                    # atualiza listas para nova vizinhança
                    indices_in = [i for i, v in enumerate(pedidos_atual) if v == 1]
                    indices_out = [i for i, v in enumerate(pedidos_atual) if v == 0]
                    break
                else:
...
if __name__ == "__main__":
    ...
    pedidos_wave, corredores_wave = ils(pedidos, corredores, lb, ub, max_iter=10000, max_tempo_segundos=LIMITE_TEMPO)
